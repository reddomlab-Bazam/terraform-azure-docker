...
# Tunnel Configuration - Fixed service URLs
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "monitoring" {
  ...
  config {
    # Wazuh ingress rule - correct Kubernetes service
    ingress_rule {
      hostname = var.wazuh_domain
      service  = "https://wazuh-dashboard.wazuh.svc.cluster.local:443"
      origin_request {
        no_tls_verify = true  # For self-signed certificates
      }
    }
    
    # Grafana ingress rule - correct Kubernetes service
    ingress_rule {
